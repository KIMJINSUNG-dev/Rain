...
| - / = 키 | 판정 타이밍 조절 — 오프셋 감소 / 증가 (5ms 단위, -200ms ~ +200ms) (= 키는 물리적으로 +가 함께 표기된 키) |
| Numpad - / Numpad + | 판정 타이밍 조절 — 오프셋 감소 / 증가 (동일 기능, 넘패드) |
| Enter | 확인 / 시작 |
| ESC | 일시정지 |
...
- **SelectScene** : 곡 선택 / 난이도 선택 / 배속 조정 / 판정 타이밍 조절
...
보면 파일 없이도 더미 데이터로 동작하도록 폴백(fallback) 처리가 적용되어 있습니다.
```
...
**판정 타이밍 조절 (오프셋)**
```
모니터 응답 지연, 오디오 출력 지연 등으로 인한 플레이어별 입력 지연을 보정합니다.
선곡 화면에서 5ms 단위로 -200ms ~ +200ms 범위 내에서 조절할 수 있습니다.
```
